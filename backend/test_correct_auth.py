...
# Login and get token
def login(username="admin", password="TestAdmin123"):  # Correct password
...
def test_customization_endpoint(token):
    # Try with trailing slash since that's what the API expects
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Even if this fails, we'll still try the upload test
            return True
    except Exception as e:
